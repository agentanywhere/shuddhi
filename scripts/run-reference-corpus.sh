...
#   python3 -m shuddhi build --registry ... --run-dir <out-dir> --build-out <dir> --lm-dir lms ...
set -euo pipefail
cd "$(dirname "$0")"
...
python3 -m shuddhi check --registry "$REG"
...
echo "$SHARDS" | tr ' ' '\n' | xargs -P 2 -I{} sh -c '
  ...
  python3 -m shuddhi train-lm --registry "${REG:-configs/reference-sangraha.json}" --shard "$1" --lm-dir lms
' _ {}
...
for s in $SHARDS; do echo "$s"; done | xargs -P 2 -I{} sh -c '
  ...
  python3 -m shuddhi run --registry "${REG:-configs/reference-sangraha.json}" --shard "$shard" --out "$out" \
...
echo "== phase 3: merge =="
python3 -m shuddhi merge --registry "$REG" --out "$OUT"
